...
public class BindUtil {
    ...
    public static void bind(UUID uuid, String name, long qqAccount) throws SQLException {
        ...
        sqlManager.executeSQLBatch("use " + QQBindPlugin.INSTANCE.getConfig().getString("database.database") + ";");
        sqlManager.createReplace(QQBindPlugin.INSTANCE.getConfig().getString("database.table"))
    ...
    public static void unBind(UUID uuid) throws SQLException {
        SQLManager sqlManager = BindUtil.sqlManager;
        sqlManager.executeSQLBatch("use " + QQBindPlugin.INSTANCE.getConfig().getString("database.database") + ";");
        sqlManager.createDelete(QQBindPlugin.INSTANCE.getConfig().getString("database.table"))
    ...
    public static long getBind(UUID uuid) {
        ...
        sqlManager.createQuery()
                .inTable(QQBindPlugin.INSTANCE.getConfig().getString("database.table"))
                ...
                .build().execute(sqlQuery -> {
                    ...
                    QQBindPlugin.INSTANCE.getLogger().log(Level.SEVERE, "Failed to get bind!", exception);
                });
    ...
    public static boolean isWaitingToVerify(UUID uuid) {
        ...
        sqlManager.executeSQLBatch("use " + QQBindPlugin.INSTANCE.getConfig().getString("database.database") + ";");
        sqlManager.createQuery()
                .inTable(QQBindPlugin.INSTANCE.getConfig().getString("database.waiting-to-verify-table"))
                .selectColumns("uuid", "name", "qq")
                .addCondition("uuid", uuid.toString())
                .build().execute((sqlQuery -> {
                    atomicBoolean.set(sqlQuery.getResultSet().next());
    ...
    public static void completeVerify(UUID uuid) throws SQLException {
        ...
        sqlManager.createQuery()
                ...
                .build().execute(sqlQuery -> {
                    ...
                    QQBindPlugin.INSTANCE.getLogger().log(Level.SEVERE, "Failed to get verify data!", exception);
                });

        long qq = atomicLong.get();
        ...
        String name = atomicReference.get();

        bind(uuid, name, qq);

        sqlManager.createDelete(QQBindPlugin.INSTANCE.getConfig().getString("database.waiting-to-verify-table"))
                .addCondition("uuid", uuid.toString())
                .build()
                .execute();
    }
    ...
    public static void verify(UUID uuid, String name, long qqAccount) throws SQLException {
        SQLManager sqlManager = BindUtil.sqlManager;
        sqlManager.executeSQLBatch("use " + QQBindPlugin.INSTANCE.getConfig().getString("database.database") + ";");
        sqlManager.createReplace(QQBindPlugin.INSTANCE.getConfig().getString("database.waiting-to-verify-table"))
                .setColumnNames("uuid", "name", "qq")
                .setParams(uuid.toString(), name.toLowerCase(), qqAccount)
                .execute();
    }
